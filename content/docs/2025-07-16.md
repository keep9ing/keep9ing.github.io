...
### 🕰️ ASP Classic (Active Server Pages)

- **출시 시기**: 1996년대 후반, .NET 이전 시대
...
### ⚙️ ASP.NET Razor (ASP.NET Core 포함)

- **출시 시기**: 2010년대 이후, .NET Framework 및 .NET Core 기반
- **언어 기반**: C# 또는 VB.NET
- **코드 구조**: HTML과 C# 코드가 Razor 문법(`@`)으로 분리되어 있음
- **특징**:
    
    - MVC 또는 Razor Pages 구조로 **비즈니스 로직과 UI 분리**
    - 객체지향 프로그래밍 지원
    - 강력한 디버깅, 보안, 성능
    - **ViewModel**을 통해 데이터 바인딩
    - HTML Helper, Tag Helper 등 다양한 도구 제공

### 🔍 핵심 차이 비교

|항목|ASP Classic|ASP.NET Razor|
|---|---|---|
|코드 스타일|절차적|객체지향, MVC|
|언어|VBScript|C#|
|유지보수|어려움|쉬움|
|성능|낮음|높음|
|보안|제한적|강력함|
|UI/로직 분리|없음|있음 (Code-behind 또는 ViewModel)|
